...
const TestPage = () => {
  const newName = EncodingLibrary.decode("d2FmZmxl")
  console.log(newName)
  return (
    <Layout>
      <section className="text-center">
        ...
        <h1>
          Hi there,{" "}
          {EncodingLibrary.decode(useQueryParam("name", "ZXBwbGljYW50Cg=="))}!
        </h1>
        <div style={{ maxWidth: `300px`, margin: `0 auto 1.45rem` }}>
          <ImageLoader
            imageToUse={EncodingLibrary.decode(
              useQueryParam("company", "d2FmZmxl")
            )}
          />
        </div>

        {EncodingLibrary.decode(useQueryParam("company", "null")) ===
        "Parallel" ? (
